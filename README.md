...
Introduced the method of **steepest descent** or simply **gradient descent**: at each step $k$, given a current estimate $x_k$ for the solution, make an improved estimate $x_{k+1} = x_k + \alpha_k r_k$, where $r_k = b - Ax_k = - \left. \nabla f \right|_{x_k}$ is the current "downhill" gradient direction.   This is a very general (albeit primitive) strategy, the starting point for many optimization algorithms on very general functions.   How do we choose the stepsize parameter $\alpha_k$ (called the "learning rate" in ML)?  For an arbitrary nonlinear function $f(x)$, this can be a tricky problem.

The most primitive choice is a *fixed* learning rate $\alpha_k = \alpha$, and a simple analysis shows that this correspondings to multiping the error $x_k - x_*$ by a fixed matrix $I - \alpha A$ at every step, which converges to zero if $\alpha < 2/\lambda_{\max}$, where $\lambda_{\max}$ is the biggest eigenvalue of $A$.  Tuning such a "hyper-parameter" α manually is cumbersome, however, and tends to lead to overly small (conservative) step sizes (slow convergence), especially if you have a more general nonlinear problem where the maximum second derivative varies considerably with $x$.

However, for the specific case of a quadratic $f$ we can solve for an "optimal" α analytically by "exact line search". In particular, suppose we are taking a step $x_{k+1} = x_k + \alpha_k d$ in some direction $d$ (e.g. $d=r_k$).  It is natural to choose the $\alpha_k$ that *minimizes* $f(x_{k+1})$.  We can solve this by setting the $\partial/\partial \alpha_k$ derivative to zero, but we can also see "geometrically" that the minimum must satisfy $d^T r_{k+1} = 0$: at the new point $x_{k+1}$, the gradient $-r_{k+1}$ must be $\perp d$, as otherwise it wouldn't minimize the value along the line.   Hence, we can show that $\alpha_k = d^T r_k / d^T A d$.  For $d = r_k \ne 0$, this implies $\alpha_k >0$: intuitively, we will always take a positive step in the downhill direction.
...
* CG generalizes to a [nonlinear conjugate-gradient](https://en.wikipedia.org/wiki/Nonlinear_conjugate_gradient_method) algorithm to (locally) minimize an arbitrary twice-differentiable f(x): the only changes are that r=-∇f is not simply b-Ax and that the successive line minimizations min f(x+αd) need to be done numerically (an “easy” 1d optimization problem … but still costly enough that people often prefer alternatives to nonlinear CG).  The "momentum" coefficient $\beta$ in conjugate gradient only depends on the ratio of $\Vert \nabla f \Vert^2$ between the current step and the previous step, so it generalizes easily to nonlinear CG (in various forms, see below).
...
Commented on two generalizations of CG to non-symmetric $Ax = b$ problems:
...
Began discussing [nonlinear least squares](https://en.wikipedia.org/wiki/Non-linear_least_squares) via the [Gauss–Newton algorithm](https://en.wikipedia.org/wiki/Gauss%E2%80%93Newton_algorithm) and its successor [Levenberg–Marquardt](https://en.wikipedia.org/wiki/Levenberg%E2%80%93Marquardt_algorithm).   The key idea is that, when minimizing $\Vert f(c) - y \Vert_2$ over $c$, we can approximate the Hessian, and hence the Newton steps, in terms of the Jacobian $f'$ alone if we assume that $\Vert f - y \Vert_2$ is small near the minimum.  Hence derived the Gauss–Newton step $c+\delta$ where $\delta$ solves $f'^T f' \delta = f'^T (y - f)$, a form of the normal equations from linear least squares.  Equivalently, we linearized $f(c+\delta) \approx f(c) + f'(c) \delta$ and then solve the resulting linear least-squares problem in $\delta$ to obtain the Gauss–Newton step.  A problem can arise, however, if the resulting step $\delta$ is so large that the linearization was invalid, which is likely if $f'$ is ill-conditioned.

**Further reading (biCG)**: A very nice overview of iterative methods for non-Hermitian problems, including BiCG can be found in these 2002 [Lecture Notes on Iterative Methods](http://www.math.uu.nl/people/vorst/lecture.html) by Henk van der Vorst (second section, starting with GMRES).

**Further reading (nonlinear least squarees)**: See, for example, the notes [*Methods for Nonlinear Least-Squares Problems*](https://www2.imm.dtu.dk/pubdb/edoc/imm3215.pdf) (2004) from DTU, which also describes several algorithms beyond Levenberg–Marquardt. Levenberg–Marquardt is related to Levenberg's method for regularized root-finding, as described in the [FNC book, section 4.6.3](https://fncbook.com/quasinewton/#levenbergs-method). Many software packages for nonlinear least-squares are available online, including as part of Julia's [NonlinearSolve.jl package](https://docs.sciml.ai/NonlinearSolve/stable/solvers/nonlinear_least_squares_solvers/) or [`scipy.optimize.curve_fit`](https://docs.scipy.org/doc/scipy/reference/generated/scipy.optimize.curve_fit.html).  See also our [18.063 Matrix Calculus course notes](https://arxiv.org/abs/2501.14787), chapter 12, on second derivatives, Hessian matrices, and bilinear maps.
